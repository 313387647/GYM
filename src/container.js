...
function createContainer(options = {}) {
  ...
  const contextBuilder = new ContextBuilder({ config, stateService, planService, mealRepository, weightRepository, memoryRepository, reminderRepository });
  const decisionEngine = new DecisionEngine({ client });
  const actionExecutor = new ActionExecutor({ mealService, weightService, workoutService, lifeService, memoryService });
  const responseComposer = new ResponseComposer({ client });
  const orchestrator = new Orchestrator({ contextBuilder, decisionEngine, actionExecutor, responseComposer, eventRepository, foodVision, mealService });
  return { config, db, client, foodVision, repositories: { mealRepository, weightRepository, workoutRepository, lifeRepository, memoryRepository, reminderRepository, eventRepository, scheduleRepository }, services: { planService, mealService, weightService, workoutService, lifeService, memoryService, stateService }, contextBuilder, decisionEngine, actionExecutor, responseComposer, orchestrator };
}
