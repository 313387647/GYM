...
class FoodVision {
  ...
  readImage(imagePath) {
    ...
    return { resolved, buffer, mime, imageHash };
  }
  imageHash(imagePath) { return this.readImage(imagePath).imageHash; }
  async analyze(imagePath, { signal } = {}) {
    const { resolved, buffer, mime, imageHash } = this.readImage(imagePath);
    const prompt = `判断是否为食物；识别主要食物、粗略份量和营养。只输出 JSON：{"is_food":true,"confidence":"high|medium|low","items":[{"name":"","amount":"","calories":0,"protein_g":0,"carbs_g":0,"fat_g":0,"confidence":"high|medium|low"}],"needs_clarification":false,"clarification_question":null}`;
    const response = await this.client.structuredJson({
      ...
      ], maxTokens: 1000, thinking: 'disabled', responseFormat: { type: 'json_object' },
      signal,
    });
