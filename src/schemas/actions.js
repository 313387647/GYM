...
const actionSchema = z.discriminatedUnion('type', [
  ...
  z.object({ type: z.literal('update_workout_rpe'), workout_id: z.coerce.number().int().positive().nullish(), selector: z.enum(['latest_today']).nullish(), rpe_score: z.coerce.number().int().min(1).max(10), notes: z.string().max(1000).nullish() }).refine((value) => value.workout_id || value.selector, 'workout_id or selector is required'),
  ...
  z.object({ type: z.literal('update_schedule_rule'), rule_id: z.string().min(1).max(100), local_time: z.string().regex(/^([01]\d|2[0-3]):[0-5]\d$/).nullish(), enabled: z.boolean().nullish(), weekdays: z.array(z.enum(['monday','tuesday','wednesday','thursday','friday','saturday','sunday'])).max(7).nullish() }).refine((value) => value.local_time !== undefined || value.enabled !== undefined || value.weekdays !== undefined, 'a schedule field is required'),
]);
